...
if (isset($_GET['release_id'])) {
    ...
    $update_sql = "UPDATE impound_records SET status = 'Released', release_date = ? WHERE record_id = ?";
    $stmt = $conn->prepare($update_sql);
    $stmt->bind_param("si", $release_date, $release_id); // Bind the date and ID parameters
    
    if ($stmt->execute()) {
        // Redirect to the same page after update
        header("Location: released_vehicles.php");
        exit();
    } else {
...
$sql = "
    SELECT impound_records.record_id, impound_records.release_date, vehicles.orcr_number, 
    ...
    WHERE impound_records.status IN ('Pending', 'Released')
";
            ...
            <div class="container-fluid">
                <div class="card card-dark">
                    <div class="card-header">
                        <h5 class="card-title">Released Vehicles</h5>
                    </div>
                    <div class="card-body">
                            ...
                            <tbody style="text-align: justify;">
                                <?php foreach ($rows as $row): ?>
                                    <tr>
                                        ...
                                        <td><?php echo htmlspecialchars(date('F j, Y', strtotime($row['release_date']))); ?></td>
